...
>julia -p <number_of_processor_to_add> repeat_paper_experiments/repeat_theory_practice_SVRG_paper_experiment_0a_decreasing.jl <boolean>
...
- the empirical total complexity v.s. mini-batch size plots are saved in ".pdf" format in the "./experiments/theory_practice_SVRG/exp0a-decreasing/figures/" folder
- the results of the simulations (mini-batch grid, empirical complexities, optimal empirical mini-batch size, etc.) are saved in ".jld" format in the "./experiments/theory_practice_SVRG/exp0a-decreasing/outputs/" folder
"""
...
save_path = "$(save_path)exp0a-decreasing/"
if !isdir(save_path)
    mkdir(save_path)
end
...
@sync @distributed for idx_prob in problems
    ...
    savename = string(probname, "-exp0a-decreasing-$(machine)-", numsimu, "-avg")
...
println("\n\n--- EXPERIMENT 0.A-DECREASING FINISHED ---")