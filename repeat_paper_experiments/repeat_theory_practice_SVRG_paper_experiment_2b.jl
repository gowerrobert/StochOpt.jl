...
skip_errors = [[700 7000 -2. 7000],  # 1)  ijcnn1_full + scaled + 1e-1                 midnight retry / FINAL
               ...
               [50000 2500 -2. 2500],  # 7)  slice + scaled + 1e-1                     100 epochs / FINAL
               [50000 1 -2. 1],  # 8)  slice + scaled + 1e-3                           100 epochs / FINAL
...
@time @sync @distributed for idx_prob in problems
    ...
    # plot_outputs_Plots(OUTPUTS, prob, options, suffix="-exp2b-$(suffix)-test", path=save_path, legendpos=legendpos, legendfont=6)
