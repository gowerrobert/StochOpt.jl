function BFGS_update!(Hout, Hin, mth::Method, dx_dy)
    ...
    c1 = (dx_dy + dot(mth.HS,mth.S)) / (dx_dy * dx_dy);
    c2 = 1 / dx_dy;
    Hout[:] = Hin .+ c1*(mth.diffpnt*mth.diffpnt').- c2*(mth.HS*mth.diffpnt' + mth.diffpnt*mth.HS');
