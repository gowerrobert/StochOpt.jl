function boot_BFGS_accel(prob::Prob, method::Method, options::MyOptions)
    ...
    method.SHS = Matrix{Float64}(I, prob.numfeatures, prob.numfeatures);  # stores the V_k's
    ...
    println("(mu, nu) =  (", round(mu, digits=3), ", ", round(nu, digits=2), ")")
    ...
    method.name = string("BFGS-a-", round(mu, digits=2), "-", round(nu, digits=2));#-",options.batchsize);
