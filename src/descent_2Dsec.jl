...
elseif(N>method.numinneriters)   # SVRG2 inner step  println("inner loop")
   sample!(1:prob.numdata,method.ind,replace=false);
    ...
    (prob.Hess_opt(method.prevx,method.ind,method.prevx-method.Sold[:]).*(method.prevx-method.Sold[:])+sig.*prob.Hess_D(method.prevx, method.ind))./
    ((method.prevx-method.Sold[:]).*(method.prevx-method.Sold[:]) .+sig) .*method.diffpnt  -
