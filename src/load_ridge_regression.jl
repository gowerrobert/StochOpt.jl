function   load_ridge_regression(X, y::Array{Float64},name::AbstractString, opts::MyOptions ; lambda =-1, normalize = true)
  ...
  prob = Prob(X,y,numfeatures,numdata,0.0,name, f_eval,g_eval,g_eval!,Jac_eval!,x->x,x->x,x->x,x->x,x->x,x->x,x->x,x->x,x->x,lambda)
