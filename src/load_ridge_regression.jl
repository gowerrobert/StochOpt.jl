function load_ridge_regression(X, y::Array{Float64}, name::AbstractString, opts::MyOptions; lambda=-1, scaling="column-scaling")
    ...
    prob = initiate_Prob(X=X, y=y, numfeatures=numfeatures, numdata=numdata, name=name, datascaling=datascaling, f_eval=f_eval, g_eval=g_eval, g_eval_mutating=g_eval!, Jac_eval_mutating=Jac_eval!, scalar_grad_eval=scalar_grad_eval, scalar_grad_hess_eval=scalar_grad_hess_eval, lambda=lambda, L=L, Lmax=Lmax, Lbar=Lbar)
    ...
    ## Try to load the strong convexity parameter of the problem, if already computed
