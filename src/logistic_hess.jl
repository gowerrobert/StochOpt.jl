...
function logistic_hessC_gradC!(X, y::Array{Float64}, w::Array{Float64}, C::Array{Int64}, lambda::Float64, batch::Int64, gC::Array{Float64}, CHC::Array{Float64})
    # Hessian of pseudo-Huber function.
    # remember! y_i^2 =1, and that is why y_i^2 terms do not appear.
    Xx  = X'*w;
    yXx = y.*Xx;
    t = logistic_phi(yXx) ;
    # t = logistic_phi( y.*X'*w) ;
    gC[:] = (1/batch)*X[C, :]*(y.*(t .- 1)).+(lambda).*w[C];
    CHC[:,:] = (1/batch)*X[C,:]*(t.*(1 .- t).*X[C, :]');
    CHC[diagind(CHC)] .+= lambda;
    # CHC[:, :] +=   lambda*Matrix{Float64}(I, length(C), length(C)) ;
end
