...
# plotting epocs per iteration
  ...
  plot(output.epocsperiter*(1:bnd)*(output.iterations/lf),(output.fs[1:bnd].-prob.fsol)./(output.fs[1].-prob.fsol), xlabel = "datapasses",  ylabel = "subopt", yscale = :log10, label  = output.name,
  linestyle=:auto,  tickfont=font(10), guidefont=font(18), legendfont =font(12), markersize = 8, linewidth =4, marker =:auto,  grid = false)
...
#  println("./figures/$(probname)-epoc.pdf")
  ...
  lf = length(output.fs);
  bnd = convert(Int64,min(ceil(datapassbnd*lf/(output.iterations*output.epocsperiter)),lf));
  plot(output.times[1:bnd],(output.fs[1:bnd].-prob.fsol)./(output.fs[1].-prob.fsol), xlabel = "time", ylabel = "subopt", yscale = :log10, label  = output.name, linestyle=:auto,   tickfont=font(12), guidefont=font(18), legendfont =font(12),  markersize = 8, linewidth =4, marker =:auto,  grid = false)
